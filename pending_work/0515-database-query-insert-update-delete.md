# [0515] Database\Query — INSERT / UPDATE / DELETE

**Phase:** Phase 3 — Database subsystem (SQLite in-memory + pure builders)
**Target source:** `src/Database/Query.php`
**Test file to create:** `tests/Unit/Database/QueryDmlTest.php`

## Scope
Cover insert()/columns()/values(), update()/set(), delete(), and clear() resetting clauses. Assert generated SQL.

## Notes & gotchas
Complements [[0510]]. No DB connection needed.
...
- [ ] One commit containing the new test (and the deletion), message style: `test: add Database\Query — INSERT / UPDATE / DELETE`.
