...
use Awf\Mvc\Model as MvcModel;

use Awf\Platform\Joomla\Helper\ComponentConfig;
use Awf\Platform\Joomla\Helper\Helper;
use Awf\Text\Text;
use JUpdater, JUpdate, JLoader, JInstallerHelper, JFile, JFolder, JInstaller, JFactory;
...
ENDBLOCK;
	...
	/**
	 * Public constructor. Initialises the protected members as well. Useful $container['config'] array keys:
	 *
	...
	public function __construct(Container $container)
	{
		...
		if (defined('_JEXEC') && version_compare(JVERSION, '2.5.0', 'lt'))
		{
			JLoader::import('joomla.updater.updater');
		}

		// Get an instance of the updater class under Joomla!
		if (defined('_JEXEC'))
		{
		...
		elseif (isset($this->container['extension_name']))
		{
			$this->component = 'com_' . $this->container->extension_name;
		}
		...
		{
			$this->component = 'com_' . $this->container->application_name;
		}

		// Get the component description
		if (isset($config['update_component_description']))
		{
			$this->componentDescription = $config['update_component_description'];
		}
		else
		{
			// Try to auto-translate (hopefully you've loaded the language files)
			$this->componentDescription = Text::_($this->component);
		}
		...
		{
			$this->commonKey = $this->container->application_name;
		}
	...
	{
		// Not Joomla!? No update site refresh.
		if (!defined('_JEXEC'))
		{
			return;
		}
	...
	{
		// Outside Joomla!? It's not like you've got a choice.
		if (!defined('_JEXEC'))
		{
			return 'classic';
		}
		...
		{
			$method = $this->container->appConfig->get('update_method', 'joomla');
		}
	...
	{
		$db = $this->container->db;
		$configHelper = new ComponentConfig($this->container);
		...
		$timeout = 3600 * $configHelper->getComponentConfigurationValue('com_installer', 'cachetimeout', '6');
		...
		if (class_exists('JUpdate'))
		{
			...
			{
				$data = $update->get('downloadurl')->_data;
				$url  = trim($data);
	...
	protected function loadUpdatesClassic($force = false)
	{
		...
		if (!$force)
		{
			...
			{
				$timeout = 3600 * $this->container->appConfig->get('updates_timeout', '6');
			}
		...
		foreach ($updates as $update)
		{
			...
			// Wrong platform?
			$targetPlatform = 'php';
			$targetPlatform = defined('_JEXEC') ? 'joomla' : $targetPlatform;
			$targetPlatform = defined('WPINC') ? 'wordpress' : $targetPlatform;

			if ($update['targetplatform']['name'] != $targetPlatform)
			{
				continue;
			}

			// Get the target platform version
			...
			if (($jVersionShort !== '0.0') && ($jVersionShort != $targetVersionShort))
			{
				continue;
			}
			...
			if (isset($update['ars-phpcompat']))
			{
				...
				foreach ($update['ars-phpcompat'] as $entry)
				{
					...
					if (version_compare($targetPHPVersion, $phpVersion, 'gt'))
					{
						continue;
					}
	...
	private function updateComponent()
	{
		if (!defined('_JEXEC'))
		{
			return "Automatic extension updates only work with Joomla!.";
		}
